...
**New models (v1.0.74–v1.0.75)**: Recent releases added support for **Claude Opus 5** (v1.0.75) and **gemini-3.6-flash** (v1.0.74). These join the existing roster of Anthropic, Google, and OpenAI models available in the picker.

**Session-scoped model override** *(v1.0.72+)*: Use `/model --session` (or `/model -s`) to change the model, reasoning effort, or context window for just the current session without touching your global or repository settings. The change reverts to your saved preferences when you open a new session:

```
/model --session claude-opus-5         # use Opus 5 just for this session
/model -s gemini-3.6-flash             # switch to Gemini Flash for the session
/model -s                              # open the picker in session-scoped mode
```

**Plan mode model picker** *(v1.0.74+)*: Use `/model plan` (or `/model --plan`) to choose a dedicated model that is used only when the agent is operating in plan mode. Pass a model ID to set it, `off` to clear it, or omit the ID to open the interactive picker. The plan-mode model reverts to the session model when you leave plan mode:

```
/model plan claude-opus-5    # use Opus 5 while in plan mode
