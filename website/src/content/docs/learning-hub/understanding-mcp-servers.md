...
> **Open Plugin Spec v1 support (v1.0.74+)**: Copilot CLI now accepts MCP server configurations written in the **Open Plugin Spec v1** format in addition to the standard `servers` object. This means `mcp.json` files generated by tools or services following that spec work without conversion. The CLI also reads `mcp.json` from the project root as a recognized config location, giving you flexibility in where you place your server definitions.
