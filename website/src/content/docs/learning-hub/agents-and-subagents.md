...
**Are multi-turn subagents available by default?**

Yes. As of v1.0.74, multi-turn subagents are always enabled — you can send follow-up messages to running subagents in `/tasks` without any configuration. Their timelines now show every prompt and response in the correct order after reopening, and the timeline identifies whether each prompt came from the main agent or another subagent, making it easier to trace complex orchestrations.
