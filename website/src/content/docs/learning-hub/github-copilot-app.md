...
If you're already in a Copilot CLI session and want to switch to the desktop app, use the `/app` command:

```bash
/app
```

This opens your current CLI session directly in the GitHub Copilot app (requires Copilot app 1.1.3 or later). The app will navigate to the same repository and session context rather than landing on the Home screen.
