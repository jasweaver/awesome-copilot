...
> **`stop_hook_active` flag (v1.0.72+)**: If an `agentStop` hook always returns a blocking exit code, the CLI would normally loop indefinitely. As a safety measure, the CLI ends the turn after **8 consecutive blocks** and passes a `stop_hook_active: true` flag in the hook's JSON input on subsequent invocations. Use this flag in your hook script to detect the forced-continuation state and self-limit:
>
> ```bash
> #!/usr/bin/env bash
> INPUT=$(cat)
> if echo "$INPUT" | jq -e '.stop_hook_active == true' > /dev/null 2>&1; then
>   # Already in forced continuation — let it through to avoid an infinite loop
>   exit 0
> fi
