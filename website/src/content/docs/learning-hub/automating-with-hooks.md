...
> **`agentStop` safety limit (v1.0.72+)**: If an `agentStop` hook always blocks (exits non-zero on every invocation), the CLI will end the turn after **8 consecutive blocks** to prevent an infinite loop. Hooks also receive a `stop_hook_active` flag in their JSON input so they can detect a forced continuation and self-limit:
>
> ```bash
> #!/usr/bin/env bash
> INPUT=$(cat)
...
>
> # Back off if we're already in a forced-continuation loop
