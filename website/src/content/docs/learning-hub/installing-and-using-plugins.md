...
*(v1.0.76+)* You can now **enable or disable** individual plugins, instructions, agents, LSP servers, and hooks without uninstalling them. This is useful when you want to temporarily turn off a plugin for a specific project or troubleshoot conflicts without losing your configuration.

Use the `/plugins` command in an interactive Copilot session to open the plugin management interface:

```
/plugins
```

From there, you can toggle any plugin, agent, instruction, LSP server, or hook on or off. Disabled components are preserved on disk but not loaded into the session. Re-enable them at any time from the same interface.

This also makes it easier to experiment with different combinations of agents and hooks without permanently removing them from your setup.
