...
> **Auto-updates (v1.0.78+)**: First-party plugins (those installed from the official Awesome Copilot marketplace) automatically update to their latest published version at session start, so you always get the newest agents, skills, and hooks without running `copilot plugin update` manually. Third-party and local plugins are not auto-updated.
