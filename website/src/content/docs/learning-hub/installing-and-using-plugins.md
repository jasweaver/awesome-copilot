...
> **Auto-updates (v1.0.78+)**: First-party plugins (those from the `copilot-plugins` official marketplace) **automatically update to the latest version at session start**. You no longer need to run `copilot plugin update` manually for official plugins.

### Enabling and Disabling Plugins

*(v1.0.76+)* You can enable or disable individual plugins, instructions, agents, LSP servers, and hooks directly from the `/plugins` dialog without uninstalling them:

```
/plugins          # open the plugins manager
```

In the dialog, select a component and toggle it on or off. Disabled plugins remain installed but are excluded from the current and future sessions until re-enabled. This is useful for temporarily switching off a plugin without losing your configuration.
